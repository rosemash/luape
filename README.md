...
Usage: `luape.exe <source OR file:source.lua> <output.exe>`

For example, if you have a file called hello.lua in the same directory as luape that you wish to compile into hello.exe: `luape.exe file:hello.lua hello.exe`. If the first argument doesn't begin with `file:` it will be interpreted as the source code.
